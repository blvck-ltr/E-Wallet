...
### 1. Clone Repository
```shell
git clone https://github.com/blvck-ltr/E-Wallet
cd E-Wallet
```
...
make
```
Câu lệnh này sẽ tạo ra một file thực thi có tên "E-Wallet" trong thư mục gốc của chương trình.
```shell
g++ src/main.cpp -o E-Wallet -std=c++11 -lssl -lcrypto
```
### 4. Chạy Chương trình
```shell
./E-Wallet
...
- Lưu trữ dữ liệu: Các file "users.txt" và "wallets.txt" (cùng các file ".bak") sẽ được tạo/cập nhật trong cùng thư mục bạn chạy file "E-Wallet".

## Cấu trúc chương trình
```
E-Wallet/
│
├── class/                # Thư mục chứa các file Header (.h)
│   │
...
│
├── data/                 # Thư mục chứa file dữ liệu (Cần tạo thư mục này thủ công. File users.txt, wallets.txt)
|
├── data_backup/           # Thư mục chứa file dữ liệu backup
│
